...
# P_cgm = n_cgm*kB*T_cgm
# Lambda_T_cgm = 1.960000e-22
# t_cool_cgm = (gamma*P_cgm)/(gm1*n_cgm**2*Lambda_T_cgm)
# print(f"rho cgms = {rho_cgm:.2e}")
# print(f"T_cgm = {T_cgm:.2e}")
# print(f"P cgms = {rho_cgm*kB*T_cgm/(mu*mp):.2e}")
# print(f"t_cool_cgm = {t_cool_cgm:.2e} s \t {t_cool_cgm/s_yr:.2e} yr")
...
T_b = P_cgm/(n_b*kB)
r_b = 1*pc
V_b = (4*np.pi*(r_b**3))/3.0
M_b = n_b*mp*mu*V_b
print(f"T_b = {T_b:.2e}K")
print(f"M_b = {M_b/Msun:.2e}Msun")