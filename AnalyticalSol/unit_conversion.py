...
# P_b     = n_b*kB*T_b
P_b     = P_cgm
T_b     = (P_cgm*mu*mp)/(rho_b*kB)
cs_b    = np.sqrt(kB*T_b/(mu*mp))
...
print(f"Black Hole Mass     = {M_bh/Msun:.2e} M_sun\t\t = {M_bh/M_code:.2e} M_code")
...
print("="*10,"Bubble Parameters in Code Units","="*76)
print(f"Bubble Radius       = {r_b:.2e} cm \t\t = {r_b/L_code:.2e} L_code")
print(f"Bubble Density      = {rho_b:.2e} g/cm^3 \t\t = {rho_b/rho_code:.2e} rho_code")
print(f"Bubble Temp         = {T_b:.2e} K \t\t = {T_b/T_code:.2e} T_code")
print(f"Bubble Mass         = {M_b/Msun:.2e} Msun \t\t = {M_b/M_code:.2e} M_code")
print(f"Bubble Pressure     = {P_b:.2e} dyne/cm^2 \t = {P_b/P_code:.2e} P_code")
print(f"Bubble Sound Speed  = {cs_b:.2e} cm/s \t\t = {cs_b/v_code:.2e} v_code")
